...
const ShowModal = ({
  ...
  const getData = async () => {
    try {
      ...
      if (showResponse.success) {
        console.log('shows of theatre', showResponse.data)
  ...
  const handleDelete = async (showId) => {
    try {
      ...
      if (response.success) {
        console.log('deleting show byshow id', response.message)
        message.success(response.message);
        getData();
      } else {
        message.error(response.message);
            ...
            <Button
              onClick={() => {
                ...
                form.setFieldsValue({
                    ...
                    date: moment(data.date).format("YYYY-MM-DD"),
                    time: data.time,
                    movie: data.movie._id, // this is required for Select
                    ticketPrice: data.ticketPrice,
                    totalSeats: data.totalSeats,
    });
                console.log('moviee name', data.movie.movieName)
                console.log('moviee name in show modal edit page',selectedMovie && selectedMovie.movieName);
              }}
            >
              <EditOutlined />
            </Button>
